...
(* CHANGE: Add a case for Shift expressions *)
      | Shift(deltaX, deltaY, e1) => let val ev = eval_prog (e1, env)  (* evaluate the expression then shift *)
					     in case ev of
						  ...
						  | NoPoints => ev
						  | _ => raise Impossible "bad call for shift: only for shape values"
				     end;
(* CHANGE: Add function preprocess_prog of type geom_exp -> geom_exp *)
(* preprocesses the expression to make sure that all line segments in it have the correct format *)
(* if the two ends of the line segments are really close, convert it to a point *)
(* else makes sure that the first point has the larger x value *)
(* if the x values are really close too, make sure the first point has the larger y*)
